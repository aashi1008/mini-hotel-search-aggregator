...
func (s *staticProvider) Search(ctx context.Context, city, checkin string, nights, adults int) ([]Hotel, error) {
	return s.hotels, nil
}
...
func TestAggregatorMerge(t *testing.T) {
	...
	res, err := agg.Search(ctx, "city", "2025-11-20", 1, 2)
	if err != nil {
		t.Fatal(err)
	}
	if len(res.Hotels) != 2 {
...
func TestAggregator_NormalizesAndDeduplicates(t *testing.T) {
    ...
    res, err := agg.Search(context.Background(), "NYc", "2025-12-01", 2, 2)
